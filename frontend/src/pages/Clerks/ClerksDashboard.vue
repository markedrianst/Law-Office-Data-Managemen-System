<template>
  <div class="p-6 bg-gray-100 min-h-screen">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Clerk Dashboard</h1>
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <div class="bg-white shadow-md rounded-lg p-6">
        <h2 class="text-lg font-semibold text-gray-800 mb-4">My Pending Requests</h2>
        ...
        <ul v-else>
          <li v-for="request in pendingRequests" :key="request.id" class="mb-2">
            {{ request.request_type }} - {{ request.case ? request.case.title : 'New Case' }}
          </li>
        </ul>
      </div>
      <div class="bg-white shadow-md rounded-lg p-6">
...
const fetchAssignedCases = async () => {
  try {
    const response = await api.get(`/cases?assigned_clerk_id=${user.value.id}`);
...
const fetchPendingRequests = async () => {
  try {
    const response = await api.get(`/case-requests?requested_by=${user.value.id}&status=pending`);
