  ...
  <div class="min-h-screen p-4 md:p-6 bg-slate-50" style="font-family: 'Segoe UI', sans-serif;">
    ...
    <div class="mb-6">
      <div class="flex items-center gap-3 mb-1">
        ...
        <h1 class="text-2xl font-bold tracking-tight text-[#1a4972]">Document Master</h1>
      </div>
      <p class="text-sm ml-4 pl-3 text-slate-500">Manage document types used across cases</p>
    </div>
      ...
      <div class="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
        ...
        <div class="flex flex-wrap gap-2 sm:gap-3">
          <select v-model="filterCategory" @change="handleFilterChange"
            class="flex-1 sm:flex-none px-3 py-2.5 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-[#1a4972] text-slate-600 min-w-[150px]">
            <option value="">All Categories</option>
            <option v-for="c in ALLOWED_CATEGORIES" :key="c" :value="c">{{ c }}</option>
          </select>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <table class="min-w-full">
          <tbody class="divide-y divide-slate-50">
            <tr v-for="i in 6" :key="i" class="animate-pulse">
              ...
              <td class="px-4 py-4"><div class="h-4 w-44 bg-slate-200 rounded mb-2"></div></td>
              <td class="px-4 py-4"><div class="h-6 w-28 bg-slate-200 rounded-lg"></div></td>
        ...
        <table class="min-w-full">
          ...
          <tbody class="divide-y divide-slate-50">
            <tr v-for="doc in documents" :key="doc.id"
              ...
              :class="{ 'opacity-60': isLastItem(doc) }">

              <!-- Sort Order badge -->
              <td class="px-4 py-4">
                <div class="flex items-center gap-1.5">
                  <span class="inline-flex items-center justify-center w-7 h-7 rounded-lg text-xs font-bold"
                    :class="isLastItem(doc)
                      ? 'bg-slate-100 text-slate-400 ring-1 ring-slate-200'
                      : 'bg-[#1a4972]/10 text-[#1a4972]'">
                    {{ doc.sort_order }}
                  </span>
                  <span v-if="isLastItem(doc)"
                    class="text-[10px] font-semibold text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded-md ring-1 ring-slate-200">
                    LAST
                  </span>
                </div>
              </td>
              ...
              <td class="px-4 py-4">
                <div class="flex items-center gap-2.5">
                  <div class="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0"
                    :class="getCategoryColor(doc.category).avatarBg">
                    <svg class="w-4 h-4" :class="getCategoryColor(doc.category).avatarIcon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                  </div>
                  <div>
                    <p class="text-sm font-semibold text-slate-800">{{ doc.type }}</p>
                    <p v-if="isLastItem(doc)" class="text-xs text-slate-400 mt-0.5 italic">Always last — new documents insert before this</p>
                  </div>
                </div>
              </td>
              ...
              <td class="px-4 py-4">
                <span v-if="doc.category"
                  ...
                  :class="getCategoryColor(doc.category).badge">
                  {{ doc.category }}
                </span>
              ...
              <td class="px-4 py-4">
                <span class="px-2.5 py-1 text-xs font-semibold rounded-lg"
                  :class="doc.requires_approval
                    ? 'bg-violet-50 text-violet-700 ring-1 ring-violet-200'
                    : 'bg-slate-100 text-slate-500 ring-1 ring-slate-200'">
                  {{ doc.requires_approval ? 'Required' : 'Not Required' }}
                </span>
              </td>
              ...
              <td class="px-4 py-4">
                <div class="flex items-center gap-1">
                  ...
                  <button @click="confirmToggle(doc)"
                    class="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-colors"
                    :class="doc.is_active
                      ? 'text-amber-600 hover:bg-amber-50'
                      : 'text-emerald-600 hover:bg-emerald-50'">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path v-if="doc.is_active" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 9v6m4-6v6m7-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
            ...
            <tr v-if="!isLoading && documents.length === 0">
              <td :colspan="columns.length" class="px-6 py-16 text-center">
                <div class="flex flex-col items-center">
                  ...
                  <p class="text-sm font-semibold text-slate-700 mb-1">No document types found</p>
      ...
      <!-- Info banner -->
      <div v-if="!isLoading && documents.length > 1" class="px-4 py-2.5 border-t border-slate-100 bg-blue-50/40">
        <p class="text-[11px] text-slate-500 flex items-center gap-1.5">
          ...
          New documents are automatically assigned sort order
          <span class="font-semibold text-[#1a4972]">{{ computedNewSortOrder }}</span>
          and inserted before
          <span class="font-semibold text-slate-700">"{{ lastDocName }}"</span> which always stays last at sort order 9999.
        </p>
      </div>
          ...
          <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div class="bg-gradient-to-br from-[#1a4972] to-[#0f2f4a] px-6 py-5">
              <div class="flex items-center justify-between">
                <div>
                  <h2 class="text-lg font-bold text-white">{{ isEditing ? 'Edit Document Type' : 'New Document Type' }}</h2>
                  <p class="text-xs text-blue-200 mt-0.5">
                    {{ isEditing
                      ? 'Update details below'
                      : `Will be assigned sort order ${computedNewSortOrder} — inserted before "Others" (9999)`
                    }}
                  </p>
                </div>
            ...
            <!-- Sort order preview (create mode only) -->
            <div v-if="!isEditing && allDocuments.length > 0" class="mx-6 mt-5 mb-1 px-4 py-3 rounded-xl bg-blue-50 border border-blue-100 flex items-start gap-3">
              <svg class="w-4 h-4 text-[#1a4972] mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4M17 8v12m0 0l4-4m-4 4l-4-4"/>
              </svg>
              <div>
                <p class="text-xs font-semibold text-[#1a4972]">Auto Sort Order: {{ computedNewSortOrder }}</p>
                <p class="text-[11px] text-slate-500 mt-0.5">
                  Inserted before <span class="font-medium">"{{ lastDocName }}"</span> which stays permanently at sort order 9999.
                  No other documents are shifted — each new document takes the next available slot.
                </p>
              </div>
            </div>
            ...
            <div class="p-6 space-y-4 max-h-[65vh] overflow-y-auto">
              ...
              <div>
                ...
                <input v-model="form.type" type="text" placeholder="e.g. Affidavit"
                  ...
                  :class="errors.type ? 'border-red-300 bg-red-50' : 'border-slate-200 bg-slate-50'" />
              ...
              <div>
                ...
                <select v-model="form.category"
                  class="w-full px-3.5 py-2.5 text-sm border rounded-xl focus:outline-none focus:border-[#1a4972] focus:ring-2 focus:ring-[#1a4972]/10 text-slate-700 transition-all"
                  :class="errors.category ? 'border-red-300 bg-red-50' : 'border-slate-200 bg-slate-50'">
              ...
              <div class="flex flex-wrap gap-6 pt-1">
                <label class="flex items-center gap-2.5 cursor-pointer">
                  <div class="relative">
                    <input type="checkbox" v-model="form.requires_approval" class="sr-only peer" />
                    <div class="w-10 rounded-full transition-colors peer-checked:bg-violet-600 bg-slate-300 peer-focus:ring-2 peer-focus:ring-violet-300" style="height:22px;"></div>
                  ...
                  <span class="text-xs font-semibold text-slate-600">Requires Approval</span>
                </label>

                <label class="flex items-center gap-2.5 cursor-pointer">
                  <div class="relative">
                    <input type="checkbox" v-model="form.is_active" class="sr-only peer" />
                    <div class="w-10 rounded-full transition-colors peer-checked:bg-[#1a4972] bg-slate-300 peer-focus:ring-2 peer-focus:ring-[#1a4972]/30" style="height:22px;"></div>
                    <div class="absolute top-0.5 left-0.5 bg-white rounded-full shadow transition-transform peer-checked:translate-x-[18px]" style="width:18px;height:18px;"></div>
                  </div>
            ...
            <div class="px-6 py-4 border-t border-slate-100 bg-slate-50/60 flex justify-end gap-2.5">
              ...
              <button @click="submitForm" :disabled="formLoading"
                ...
                {{ formLoading ? 'Saving…' : (isEditing ? 'Save Changes' : 'Create Document') }}
              </button>
            </div>
          </div>
        </div>
...
// ==================== CONSTANTS ====================
const ALLOWED_CATEGORIES = ['Pleading', 'Letter', 'Evidence', 'Court Issuance', 'Other'];

const columns = [
  { label: '#',        field: 'sort_order',        sortable: true  },
  { label: 'Type',     field: 'type',              sortable: true  },
  { label: 'Category', field: 'category',          sortable: true  },
  { label: 'Approval', field: 'requires_approval', sortable: true  },
  { label: 'Status',   field: 'is_active',         sortable: true  },
  { label: 'Actions',  field: 'actions',           sortable: false },
];

const CATEGORY_COLORS = {
  'Pleading':       { badge: 'bg-blue-50 text-blue-700 border-blue-200',           avatarBg: 'bg-blue-100',    avatarIcon: 'text-blue-600'    },
  'Letter':         { badge: 'bg-sky-50 text-sky-700 border-sky-200',              avatarBg: 'bg-sky-100',     avatarIcon: 'text-sky-600'     },
  'Evidence':       { badge: 'bg-emerald-50 text-emerald-700 border-emerald-200',  avatarBg: 'bg-emerald-100', avatarIcon: 'text-emerald-600' },
  'Court Issuance': { badge: 'bg-amber-50 text-amber-700 border-amber-200',        avatarBg: 'bg-amber-100',   avatarIcon: 'text-amber-600'   },
...
const getCategoryColor = (cat) => cat ? (CATEGORY_COLORS[cat] ?? DEFAULT_COLOR) : DEFAULT_COLOR;

// ==================== STATE ====================
const documents    = ref([]);   // paginated (display)
const allDocuments = ref([]);   // full unfiltered list sorted by sort_order ASC
...
const errors = reactive({ type: '', category: '' });
...
/**
 * RULES (identical to CourtMaster):
 * ─────────────────────────────────────────────────────────────
 * • "Others" (sort_order = 9999) is the permanent last item — it NEVER moves.
 * • All other documents start at sort_order = 1 and increment by 1.
 * • New document sort_order = (highest non-Others sort_order) + 1
 *   Always < 9999, so Others always stays last.
 * • No shifting of existing items needed.
 *
 * Backend contract (DocumentController@store):
 *   Document::create([..., 'sort_order' => $request->sort_order]);
 *   (no increment query needed)
 */
const OTHERS_SORT_ORDER = 9999;

/** The "Others" anchor — identified by sort_order 9999 OR type "Others" */
const othersItem = computed(() =>
  allDocuments.value.find(d => d.sort_order === OTHERS_SORT_ORDER || d.type === 'Others') ?? null
);

/** All documents except Others, sorted ASC */
const nonOtherDocuments = computed(() =>
  allDocuments.value
    .filter(d => d.id !== othersItem.value?.id)
    .sort((a, b) => a.sort_order - b.sort_order)
);

/** Highest sort_order among non-Others documents. Falls back to 0 so first doc gets 1. */
const maxNonOtherSortOrder = computed(() =>
  nonOtherDocuments.value.length
    ? nonOtherDocuments.value[nonOtherDocuments.value.length - 1].sort_order
    : 0
);

/** sort_order for the new document — always starts at 1, increments, capped at 9998 */
const computedNewSortOrder = computed(() =>
  Math.min(maxNonOtherSortOrder.value + 1, OTHERS_SORT_ORDER - 1)
);

const lastDocName = computed(() => othersItem.value?.type ?? 'Others');

/** True when this row is the permanent "Others" anchor */
const isLastItem = (doc) =>
  othersItem.value != null && doc.id === othersItem.value.id;
...
/** Full unfiltered list sorted by sort_order ASC — used to find the anchor and next slot */
const loadAllDocuments = async () => {
  try {
    ...
    allDocuments.value = (res.data?.data ?? []).map(DocumentService.formatDocument);
  } catch (e) { console.error('loadAllDocuments:', e); }
};
...
const debouncedSearch    = () => { clearTimeout(searchTimer); searchTimer = setTimeout(() => { currentPage.value = 1; loadDocuments(); }, 300); };
const handleFilterChange = () => { currentPage.value = 1; loadDocuments(); };
const sortBy = (field) => {
...
const previousPage = () => { if (pagination.value.current_page > 1)                          { currentPage.value--; loadDocuments(); } };
const nextPage     = () => { if (pagination.value.current_page < pagination.value.last_page) { currentPage.value++; loadDocuments(); } };
const goToPage     = (page) => { currentPage.value = page; loadDocuments(); };

// ==================== FORM ====================
const clearErrors = () => { errors.type = ''; errors.category = ''; };
...
const openEdit = (doc) => {
  isEditing.value = true; editingId.value = doc.id;
  Object.assign(form, {
    type:              doc.type,
    category:          doc.category          ?? '',
    requires_approval: doc.requires_approval ?? false,
...
const validateForm = () => {
  ...
  if (!form.type.trim()) { errors.type     = 'Document type is required'; ok = false; }
  if (!form.category)    { errors.category = 'Category is required';      ok = false; }
  return ok;
};
...
const submitForm = async () => {
  ...
  try {
    if (isEditing.value) {
      await DocumentService.update(editingId.value, { ...form });
    } else {
      // Auto-assign sort_order: starts at 1, increments with each new doc, always < 9999
      await DocumentService.store({ ...form, sort_order: computedNewSortOrder.value });
    }
    await loadDocuments();
    closeForm();
  } catch (e) {
    const errs = e?.response?.data?.errors ?? e?.errors ?? {};
    if (errs.type)     errors.type     = Array.isArray(errs.type)     ? errs.type[0]     : errs.type;
    if (errs.category) errors.category = Array.isArray(errs.category) ? errs.category[0] : errs.category;
  } finally { formLoading.value = false; }
};

// ==================== TOGGLE ACTIVE ====================
const confirmToggle = (doc) => {
  actionTarget.value   = doc;
  confirmTitle.value   = doc.is_active ? 'Deactivate Document?' : 'Activate Document?';
  confirmMessage.value = `"${doc.type}" will be ${doc.is_active ? 'hidden from case document selections' : 'available for use in cases'}.`;
  confirmBtnText.value = doc.is_active ? 'Deactivate' : 'Activate';
...
const executeToggle = async () => {
  ...
  try {
    await DocumentService.toggleActive(actionTarget.value.id);
