        ...
        <div>
          <div class="flex items-center gap-3 mb-1.5">
            ...
            <div>
              ...
              <p class="text-xs md:text-sm text-slate-500 mt-0.5">Review and manage pending document movements</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <table class="min-w-full divide-y divide-slate-200">
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="mv in visibleRows" :key="`${mv._source}-${mv.id}`"
              ...
              :class="mv.approval_status === 'PENDING' 
        ...
        <div v-for="mv in visibleRows" :key="`m-${mv._source}-${mv.id}`"
          ...
          <div class="grid grid-cols-2 gap-x-4 gap-y-2.5 mb-4">
            ...
            <div v-if="mv.handled_by">
              <p class="text-[10px] uppercase tracking-wider text-slate-500 font-black mb-1">Handled By</p>
              <p class="text-xs text-slate-800 font-semibold">{{ mv.handled_by }}</p>
            </div>
            <div>
            ...
            <div v-if="mv.purpose" class="col-span-2">
              <p class="text-[10px] uppercase tracking-wider text-slate-500 font-black mb-1">Purpose</p>
              <p class="text-xs text-slate-700 leading-relaxed">{{ mv.purpose }}</p>
            </div>
          </div>

          <!-- Approver Info -->
          <p v-if="mv.approval_status !== 'PENDING' && mv.approver?.full_name"
            class="text-xs text-slate-500 mb-3 pb-3 border-b border-slate-200">
            {{ mv.approval_status === 'APPROVED' ? '✓ Approved' : '✗ Rejected' }} by
            <span class="font-bold text-slate-700">{{ mv.approver.full_name }}</span>
          </p>
          ...
          <div v-else class="pt-3 border-t border-slate-200">
            <span class="text-xs text-slate-500 font-semibold italic">✓ Reviewed — no further action needed</span>
          </div>
        </div>
      </div>
        ...
        <div class="relative bg-white w-full sm:rounded-2xl rounded-t-2xl shadow-xl sm:max-w-lg p-6 sm:p-7 border border-slate-200">
          ...
          <div class="flex items-start gap-4 mb-5">
            <div class="w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0"
              :class="confirm.action === 'APPROVED' 
                ? 'bg-emerald-100 border-2 border-emerald-300' 
                : 'bg-red-100 border-2 border-red-300'">
              <svg class="w-7 h-7" :class="confirm.action === 'APPROVED' ? 'text-emerald-700' : 'text-red-600'"
            ...
            <div class="flex-1">
              <h3 class="text-lg font-black text-slate-900 mb-1">
                {{ confirm.action === 'APPROVED' ? 'Approve this movement?' : 'Reject this movement?' }}
              </h3>
          ...
          <!-- Movement Details -->
          <div v-if="confirm.mv" class="mb-6 p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
            <div class="flex items-center gap-2">
              <span class="text-xs font-bold text-slate-500">CASE:</span>
              <span class="text-xs font-black text-blue-700">{{ confirm.mv.case_code ?? `#${confirm.mv.case_id}` }}</span>
            </div>
            <div class="flex items-center gap-2 flex-wrap">
              <span class="text-xs font-bold text-slate-500">TYPE:</span>
              <span class="text-xs font-bold text-slate-700">{{ confirm.mv._source === 'checklist' ? '📋 Checklist' : '📁 Folder' }}</span>
              <span class="px-2 py-0.5 rounded text-xs font-bold"
                :class="confirm.mv.type === 'OUT' 
                  ? 'bg-rose-100 text-rose-700 border border-rose-300' 
                  : 'bg-emerald-100 text-emerald-700 border border-emerald-300'">
                {{ confirm.mv.type }}
              </span>
            </div>
            <div v-if="confirm.mv.checklist?.task || confirm.mv.from_to">
              <span class="text-xs font-bold text-slate-500">{{ confirm.mv.checklist?.task ? 'TASK:' : 'TO/FROM:' }}</span>
              <p class="text-xs text-slate-800 font-semibold mt-1">{{ confirm.mv.checklist?.task ?? confirm.mv.from_to }}</p>
            </div>
          </div>
          ...
          <div class="flex flex-col sm:flex-row gap-3">
            ...
            <button @click="confirmDecide"
              ...
              <svg v-if="confirm.processing" class="animate-spin w-5 h-5" viewBox="0 0 24 24" fill="none">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/>
              </svg>
...
const pagination = computed(() => store.state.pagination.approvals);
const isLoading = computed(() => store.state.isLoading);

// ── State ─────────────────────────────────────────────────────────────────────────────────
const processingId = ref(null);

// Filters - DEFAULT TO ALL STATUS
...
const loadAll = async (silent = false) => {
  ...
  try {
    ...
    // Also update global pending count for sidebar
...
watch([filterStatus, filterType, filterDirection], () => {
  currentPage.value = 1;
  loadAll();
});
...
const onVisibilityChange = () => {
  ...
  if (initialLoadDone) loadAll(true);
};
...
onMounted(() => {
  ...
  // If the store is already initialized, we show data instantly.
  // We only trigger a refresh if it's the first time.
  if (!store.state.isInitialized) {
    ...
    // Data is already in store, just start background listeners
    initialLoadDone = true;
    openChannel();
    document.addEventListener('visibilitychange', onVisibilityChange);
    startPolling();
